...
export default function HealthRecordsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        const res = await axios.put(`http://127.0.0.1:8000/api/healthrecords/${form.record_id}`, form, { withCredentials: true });
  ...
  return (
    <div className="p-6">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-2 mb-4">
        <input type="date" value={form.visit_date} onChange={e => setForm({...form, visit_date: e.target.value})} required className="border p-2"/>
        <input type="text" placeholder="Diagnosis" value={form.diagnosis} onChange={e => setForm({...form, diagnosis: e.target.value})} required className="border p-2"/>
        <input type="text" placeholder="Treatment" value={form.treatment} onChange={e => setForm({...form, treatment: e.target.value})} className="border p-2"/>
        <button type="submit" className="bg-emerald-600 text-white px-3 py-2 rounded">{isEditing ? "Update" : "Add"}</button>
      </form>
